...
### Table Level and Kind
Each repository says which table it uses with two get-only properties. The defaults are `TableLevel.Default`
(the table named by `CallerInfo.DefaultDB`) and `TableKind.Lsi`. A repository generated by `lz gen` overrides both
from its schema's `x-lz-tablelevel` and `x-lz-tablekind`, so the schema is the one place they are set:

```csharp
public partial class LeadRepo : DYDBRepository<Lead>, ILeadRepo
{
    protected override TableLevel TableLevel => TableLevel.Default; // SystemDB, TenantDB, SubtenantDB, DefaultDB or Local (tablename)
    protected override TableKind TableKind => TableKind.Gsi;         // Lsi: the level's table. Gsi: its "_gsi" twin.
}
```

- **Lsi**: the level's table, with five `ALL`-projection local secondary indexes `PK-SK1-Index` … `PK-SK5-Index`.
  Every item of a type shares one partition key value, so an LSI table caps a type at 10 GB.
- **Gsi**: the same name plus `_gsi`, with five `KEYS_ONLY` global secondary indexes of the same names and keys and
  no 10 GB cap. A list on an index queries it for keys, fetches the items with a consistent `BatchGetItem`, leaves
  out any item that is gone or whose key no longer matches the query (the index lags the table), and keeps the
  index's order. A read it cannot finish, throttled or with keys still unprocessed after its retries, answers 503;
  it never returns a short list as complete. An index cannot be read consistently, and `UseIsDeleted` is not
  supported on this path.

Subclasses also get reads by key: `ReadAsync(callerInfo, id, consistentRead)`, a consistent whole-type
`ListAsync(callerInfo, limit, consistentRead)`, and `ReadItemsAsync` / `ReadManyAsync` for many ids with
`BatchGetItem` (100 keys a call, consistent by default). They are protected so a repository that checks access in
its public `ReadAsync` cannot be read around.
...
### Table Structure
Entities share their level's table, keyed `PK` = `"{EntityType}:"` and `SK` = `"{id}:"`. The table name comes from
the caller: `CallerInfo.SystemDB`, `TenantDB`, `SubtenantDB` or `DefaultDB` for those levels, or the repository's
own `tablename` at the `Local` level. A `Gsi` repository adds `_gsi` to that name. An empty name stays empty.
