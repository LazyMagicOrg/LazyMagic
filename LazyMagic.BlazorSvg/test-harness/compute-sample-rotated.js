...
// Paths
const combinationsPath = path.join(__dirname, 'valid-combinations-sample-rotated.json');
const outputPath = path.join(__dirname, 'precomputed-rectangles-sample-rotated.json');
const testConfigPath = path.join(__dirname, 'test-config-rotated.js');
const testRunnerPath = path.join(__dirname, 'test-runner-rotated.js');
...
export const config = {
    ...
    svgPath: "../../BlazorTest.WASM/wwwroot/Level1-rotated.svg",
...
`;
    ...
    // Backup original test-config-rotated.js
    if (fs.existsSync(testConfigPath)) {
        fs.copyFileSync(testConfigPath, backupPath);
        console.log(`  Backed up original test-config-rotated.js`);
    }
...
/**
 * Restore original test-config-rotated.js
 */
function restoreTestConfig(backupPath) {
    if (fs.existsSync(backupPath)) {
        fs.copyFileSync(backupPath, testConfigPath);
        fs.unlinkSync(backupPath);
        console.log(`  Restored original test-config-rotated.js`);
    }
}
...
async function main() {
    ...
    console.log('OPTIMIZED ALGORITHM TEST - SAMPLE OF 10 COMBINATIONS (ROTATED)');
    console.log('='.repeat(80));
    console.log();
    ...
    try {
        ...
        // Step 3: Run test-runner-rotated.js
        console.log('Step 3: Running test-runner-rotated.js with OPTIMIZED algorithm...');
        ...
        console.log('Check TestResultsRotated directory for output SVG files');
        console.log('='.repeat(80));
