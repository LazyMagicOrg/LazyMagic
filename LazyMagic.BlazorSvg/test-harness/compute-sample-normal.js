...
// Paths
const combinationsPath = path.join(__dirname, 'valid-combinations-sample-normal.json');
const outputPath = path.join(__dirname, 'precomputed-rectangles-sample-normal.json');
const testConfigPath = path.join(__dirname, 'test-config-normal.js');
const testRunnerPath = path.join(__dirname, 'test-runner-normal.js');
...
`;
    ...
    // Backup original test-config-normal.js
    if (fs.existsSync(testConfigPath)) {
        fs.copyFileSync(testConfigPath, backupPath);
        console.log(`  Backed up original test-config-normal.js`);
    }
...
/**
 * Restore original test-config-normal.js
 */
function restoreTestConfig(backupPath) {
    if (fs.existsSync(backupPath)) {
        fs.copyFileSync(backupPath, testConfigPath);
        fs.unlinkSync(backupPath);
        console.log(`  Restored original test-config-normal.js`);
    }
}
...
async function main() {
    ...
    console.log('OPTIMIZED ALGORITHM TEST - SAMPLE OF 10 COMBINATIONS (NORMAL)');
    console.log('='.repeat(80));
    console.log();
    ...
    try {
        ...
        // Step 3: Run test-runner-normal.js
        console.log('Step 3: Running test-runner-normal.js with OPTIMIZED algorithm...');
        ...
        console.log('Check TestResultsNormal directory for output SVG files');
        console.log('='.repeat(80));
