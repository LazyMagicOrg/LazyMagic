...
# 2. Generate max-inscribed rectangles (15-20 minutes)
node test-runner.js
...
The max-inscribed pipeline is **completely independent** from the other pipelines:

- Can run separately at different times
- Uses different output directory (`TestResults/MaxInscribedResults/`)
...
# Max-inscribed rectangles (15-20 min)
node test-runner.js
node extract-precomputed-rectangles.js
node embed-rectangles-in-svg.js
...
```bash
# Generate max-inscribed rectangles
node test-runner.js
