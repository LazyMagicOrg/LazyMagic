...
```bash
# 1. Navigate to test-harness directory
cd "C:\Users\noaht\source\repos\_Dev\LazyMagic\LazyMagic\LazyMagic.BlazorSvg\test-harness"

# 2. Generate hollow square layouts (15-20 minutes)
node test-runner-hollowsquare.js

# 3. Extract data to JSON (~30 seconds)
node extract-precomputed-hollowsquare.js

# 4. Embed in SVG (~5 seconds)
node embed-hollowsquare-in-svg.js
...
**Total Time:** ~20 minutes (same as largest-rectangle and boardroom pipelines)

### Independent Operation

The hollow square pipeline is **completely independent** from the other pipelines:

- Can run separately at different times
- Uses different output directory (`TestResults/HollowSquareResults/`)
...
```bash
# Generate hollow square layouts
node test-runner-hollowsquare.js
