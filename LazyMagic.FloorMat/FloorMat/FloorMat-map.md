...
**Status:** ✅ **CLEAN** - Multi-project pipeline with CommonJS algorithm modules
...
| File | Purpose | Module Type | Line Count |
|------|---------|-------------|------------|
| **SvgViewerBoundaryBased.cjs** | Boundary-based + hybrid inscribed rectangle algorithm | CommonJS | ~2,407 |
| **SvgViewerOptimized.cjs** | Grid-based centroid sampling with binary search | CommonJS | ~1,746 |
| **SvgViewerInscribedRect.cjs** | Unified inscribed rectangle algorithm | CommonJS | ~630 |
| **SvgViewerBoardroom.cjs** | Boardroom layout algorithm (fixed 13ft width) | CommonJS | ~570 |
| **SvgViewerHollowSquare.cjs** | Hollow square layout algorithm | CommonJS | ~550 |
| **kdtree.cjs** | Spatial data structures (KDTree, SpatialGrid) | CommonJS | ~462 |
| **SvgViewerAlgorithms.cjs** | SVG path parsing and geometric utilities | CommonJS | ~1,340 |

**Note:** These are CommonJS modules (`.cjs`) loaded by `run-tests.js` for Node.js execution. Browser versions exist in `LazyMagic.BlazorSvg/wwwroot/` as ES6 modules.
...
| **run-tests.js** | Run layout algorithm tests (loads .cjs modules) | test-configs/*.json, valid-combinations*.json | ../TestResults/*/*.json |
...
| **embed-path-metadata.js** | Embed room section metadata as floormat:* attributes | input/{prefix}-data.json | {prefix}-with-metadata.svg |
...
| File | Purpose |
|------|---------|
| **SvgViewer.js** | Large browser component (ES6 module, 161 KB) - reference copy |
| **check-limits.cjs** | Debug tool for testing hollow square performance |
| **FloorMat-map.md** | This documentation file |

**Note:** `SvgViewer.js` is a reference copy. The active version for Blazor runtime is in `LazyMagic.BlazorSvg/wwwroot/`.

---

## Total File Count

- **Algorithm Files:** 7 CommonJS modules (.cjs)
- **Core Pipeline Scripts:** 9 files (ES6 modules)
...
- **Total Essential Files:** ~28 files (excluding node_modules and generated output)
