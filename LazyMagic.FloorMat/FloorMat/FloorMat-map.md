...
|------|---------|-------|--------|
| **compute-all-combinations.js** | Generate all valid room combinations | input/Rooms.json | valid-combinations.json |
| **process-all.js** | Multi-project orchestrator - auto-detects and processes all projects in FloorMat/input/ | input/*.svg, input/*-Rooms.json | output/*/Level1-output.svg |
| **process-external.js** | Process projects in external directories | <external-dir>/input/*.svg, *-Rooms.json | <external-dir>/output/*-output.svg |
...
| **Level1-Rooms.json** | Graph connectivity data (sections and joins) | compute-all-combinations.js |
...
output/
└── Level1-output/
    ├── Level1-valid-combinations.json   (generated from Level1-Rooms.json)
    ├── ComputedLayouts/
...
- **Utility Files:** 2 files
- **Input Directory:** 2 files (Level1.svg, Level1-Rooms.json)
...
# 2. Look for matching Rooms files (e.g., Level1-Rooms.json)
...
└── {ProjectName}-Rooms.json             (User provides: Graph connectivity)
...
- `node compute-all-combinations.js <prefix>` - Generate all 251 valid combinations from [prefix]-Rooms.json
...
1. Project Auto-Detection
   ...
   ├─→ Finds: Level1.svg
   └─→ Finds: Level1-Rooms.json

2. Combination Generation (per project)
   compute-all-combinations.js
   ├─→ Reads: input/Level1-Rooms.json (graph connectivity)
...
2. `input/Rooms.json` - Graph connectivity data for combination generation
3. `input/Level1.svg` - Project source SVG
4. `input/Level1-Rooms.json` - Project-specific rooms data
5. `process-all.js` - Multi-project orchestrator (internal)
6. `process-external.js` - Multi-project orchestrator (external)
7. `extract-precomputed-project.js` - Per-project extraction
8. `embed-project.js` - Per-project embedding
...
**Input:** `input/Rooms.json` - Graph with sections (rooms/aisles/crossings) and joins
**Output:** `valid-combinations.json` - 251 valid combinations
**Rules Applied:**
1. Adjacency Constraint - Shared aisles must be selected
2. Single section must be Room
3. Aisle can include single room
